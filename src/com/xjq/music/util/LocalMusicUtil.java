...
public class LocalMusicUtil {
	...
	public static List<MusicInfomation> getLocalAudioList(Context context) {
		if (DEBUG)
			Log.i(TAG,
					"	--->LocalMusicUtil--->getLocalAudioList ###MediaStore.Audio.Media.EXTERNAL_CONTENT_URI= "
		...
		if (DEBUG)
			Log.i(TAG,
					"	--->LocalMusicUtil--->getLocalAudioList ###cursor.moveToNext()= "
							+ cursor.moveToNext());
		...
		if (DEBUG)
			Log.i(TAG,
					"	--->LocalMusicUtil--->getLocalAudioList return######musicList= "
							+ musicList);
		if (DEBUG)
			Log.i(TAG,
