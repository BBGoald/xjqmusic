...
public class LyricView extends TextView {
	...
	/**
	 * 下面三个构造函数必须要都重载，否则在activity_play_detailxml文件里的<com.xjq.music.lyric.
	 * LyricView会报错。
	 * 
	 * @param context
	 */
	...
	public void setLyricObject(TimedTextObject lyricObject) {
		if (DEBUG)
			Log.i(TAG, "	--->LyricView--->setLrcObject before###lyricObject= "
					+ lyricObject);
		...
		if (DEBUG)
			Log.i(TAG, "	--->LyricView--->setLrcObject after###list= " + list
					+ "	i= " + i);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (list != null) {
			if (DEBUG)
				Log.i(TAG,
						"	--->LyricView--->onDraw ###list.get(index).getTextContent()= "
								+ list.get(index).getTextContent() + "	index= "
								+ index);
		}
		if (canvas == null) {
			return;
		...
		if (list == null || list.size() == 0) {
			if (DEBUG)
				Log.i(TAG, "	--->LyricView--->onDraw ###list= null");
			canvas.drawText("There is no Lyric@_@\n木有歌词捏-_><_-", mX, middleY,
					painCurrentIndex);
			return;
		}
		...
		canvas.drawText(list.get(index).getTextContent(), mX, middleY,
				painCurrentIndex);
		...
		for (int i = index - 1; i > 0; i--) {
			...
			if (DEBUG)
				Log.i(TAG,
						"	--->LyricView--->onDraw ###list.get(i).getTextContent()= "
								+ list.get(i).getTextContent());
			canvas.drawText(list.get(i).getTextContent(), mX, tempY,
					paintPreOrLaterIndex);
		}
	...
	public long updateIndex(Lyric lyric) {
		...
		if (DEBUG)
			Log.i(TAG,
					"	--->LyricView--->updateIndex ###lyric.endTime.mSeconds - lyric.startTime.mSeconds= "
